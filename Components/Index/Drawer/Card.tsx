...
const CV_URL = "https://drive.google.com/uc?export=download&id=1Z7IlpPZKeZfL7QZdNQYUO9eH0sn1VZeD";
    ...
    const openGitHub = () => {
      ...
      const downloadCV = async () => {
        try {
          ...
          // İndirme başarılıysa kullanıcıya bilgi ver
          Alert.alert("Download Complete", "CV successfully downloaded to your device.");
      
        } catch (error) {
          // Eğer hata varsa kullanıcıya bildir
...
<View style={styles.infoContainer}>
  ...
  <Text style={styles.infoText}>22 years old</Text>
    ...
    <TouchableOpacity style={styles.sView} onPress={openGitHub}>
      <FontAwesome style={{alignSelf:'center',marginTop:12}} name="github" size={40} color="black" />
      <Text style={styles.linkText}>GitHub Profile</Text>
    </TouchableOpacity>
     
    <TouchableOpacity style={[styles.sView, styles.linkedin]} onPress={openLinkedIn}>
        <FontAwesome style={styles.icon} name="linkedin-square" size={40} color="white" />
        <Text style={styles.linkText}>LinkedIn Profile</Text>
      </TouchableOpacity>
      <TouchableOpacity style={[styles.sView]} onPress={callMe}>
      <MaterialIcons style={styles.icon} name="phone-callback" size={30} color="black" />
